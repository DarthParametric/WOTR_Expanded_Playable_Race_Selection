...
namespace DP_WOTR_PlayableRaceExp.Utilities {
    public static class BlueprintTools {
		...
		public static T GetModBlueprintReference<T>(ModContextBase modContext, string name) where T : BlueprintReferenceBase {
            ...
            if (assetId == null) { Main.RaceExpContext.Logger.LogError($"COULD NOT LOAD MOD BLUEPRINT REFERENCE: {name} - {typeof(T)}"); }
        ...
        public static T GetModBlueprint<T>(ModContextBase modContext, string name) where T : SimpleBlueprint {
            ...
            if (value == null) { Main.RaceExpContext.Logger.LogError($"COULD NOT LOAD MOD BLUEPRINT: {name} - {typeof(T)}"); }
            return value as T;
        }
        ...
        public static T GetBlueprint<T>(BlueprintGuid id) where T : SimpleBlueprint {
            ...
            T value = asset as T;
            if (value == null) { Main.RaceExpContext.Logger.LogError($"COULD NOT LOAD BLUEPRINT: {id} - {typeof(T)}"); }
            return value;
        }
        public static void AddBlueprint(ModContextBase modContext, SimpleBlueprint blueprint) {
            AddBlueprint(modContext, blueprint, blueprint.AssetGuid);
        }
        public static void AddBlueprint(ModContextBase modContext, SimpleBlueprint blueprint, string assetId) {
            var Id = BlueprintGuid.Parse(assetId);
            AddBlueprint(modContext, blueprint, Id);
        }
        public static void AddBlueprint(ModContextBase modContext, SimpleBlueprint blueprint, BlueprintGuid assetId) {
        ...
        public static void TryExportBlueprint(SimpleBlueprint bp, string path) {
            ...
            try {
                ...
                Main.RaceExpContext.Logger.LogError($"Failed to export {bp.AssetGuid}-{bp.name}");
            }
        }
    }
}