    ...
    public class SettingGroup : IDisableableGroup {
        ...
        public virtual bool IsEnabled(string key) {
            if (!Settings.TryGetValue(key, out SettingData result)) {
                if (!hasDumpedKeys) {
                    hasDumpedKeys = true;
                    Main.RaceExpContext.Logger.Log($"DUMPING KEYS: {key}");
                    foreach (var entry in Settings) {
                        Main.RaceExpContext.Logger.Log($"{entry.Key}");
                    }
                }
                Main.RaceExpContext.Logger.LogError($"COULD NOT FIND SETTING KEY: {key}");
                return false;
            }
